...
rpy3yr_lasso_pred <- predict(rpy3yr_lasso, s= rpy3yr_rr_bestlam, newx = rpy3yr_test_x)
...
rpy3yr_lasso_full_fit <- predict(rpy3yr_lasso_out, type = "coefficients", s = rpy3yr_rr_bestlam)
rpy3yr_rr_coef <- as.matrix(rpy3yr_lasso_full_fit)
...
cdr3_lasso_pred <- predict(cdr3_lasso, s= cdr3_rr_bestlam, newx = cdr3_test_x)
cdr3_lasso_test_mse <- mean((cdr3_lasso_pred - cdr3_test_y)^2)

#full data
cdr3_lasso_out <- glmnet(cdr3_x,cdr3_y, alpha = 1, intercept = FALSE)
cdr3_lasso_full_fit <- predict(cdr3_lasso_out, type = "coefficients", s = cdr3_rr_bestlam)
cdr3_rr_coef <- as.matrix(cdr3_lasso_full_fit)
