...
\begin{figure}
\caption{Barplot of Mean RPY_3YR_RT on REGION Levels}
\centering
...
is a barplot of average value of rpy_3yr_rt for each region. rpy_3yr_rt is defined as a fraction of repayment cohort who are not in default, and with loan balances that have declined three years since entering repayment, excluding enrolled and military deferment from calculation. (rolling averages) and each university is divided into 9 regions: 0	U.S. Service Schools, 1New England (CT, ME, MA, NH, RI, VT), 2	Mid East (DE, DC, MD, NJ, NY, PA), 3	Great Lakes (IL, IN, MI, OH, WI),4	Plains (IA, KS, MN, MO, NE, ND, SD), 5	Southeast (AL, AR, FL, GA, KY, LA, MS, NC, SC, TN, VA, WV), 6	Southwest (AZ, NM, OK, TX), 7	Rocky Mountains (CO, ID, MT, UT, WY), 8	Far West (AK, CA, HI, NV, OR, WA), 9	Outlying Areas (AS, FM, GU, MH, MP, PR, PW, VI). Our graph is not showing region 0 because there is one university that is under region 0 and it did not have a value for rpy_3yr_rt. On barplot it shows that region 1 and 2 have highest repayment rate(0.73 and 0.68, respectively) and region 5 and 6 have lowest repayment rate (0.54)


\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{../images/eda/rpy3yr_control_barplot}
\caption{Barplot of Mean RPY_3YR_RT on CONTROL Levels}
\end{figure}

rpy3yr_control_barplot:
is another barplot that shows average value of rpy_3yr_rt for each school type. Under CONTROL column, each school is divided into three categories: 1 = Public, 2 = Private nonprofit, 3 = Private for-profit. It shows that Private for nonprofit universities has highest average repayment rate with 0.77. On the right side, rpy3yr_insttype_histogram helps for the better understanding of rpy3yr_control_barplot. rpy3yr_insttype-histogram shows density graph and histogram of each institution's repayment rate.

\begin{figure}
\caption{Barplot of Mean RPY_3YR_RT on ICLEVEL Levels}
\centering
\includegraphics[width=0.6\textwidth]{../images/eda/rpy3yr_iclevel_barplot.png}
\end{figure}

rpy3yr_iclevel_barplot.png:
is a last barplot that shows average value of rpy_3yr_rt for each level of institution. Uncer ICLEVEL, each school is divided into three categories: 1 = 4-year,2 = 2-year, 3 = Less-than-2-year. It shows that 4-year institution has the highest average repayment rate with 0.69. On the right side of the barplot, there is a histogram/desnity plot of each institution's repayment rate for detailed analysis.

rpy3yr_tuition_scatter:
is a three different scatterplots of Tuition fee vs Repayment Rate. In the data set, there are three different information of Tuition fee under cost category. First graph is a Net tuition revenue per full-time equivalent studnet vs repayment rate. Second graph is a In-stat tuition and fee vs repayment rate and last one is out-of-state tuition and fees vs repayment rate. There is highest correlation between out-of-state vs repayment rate.

netprice_pub_priv_rpy3yr_scatter:
The first graph is a scatterplot of 3 yr repayment rate vs average net price for public institutions and below, it has a scatterplot of 3 yr repayment rate vs average net price for private instutitions. It shows higher slope for public institution but for both graph, it shows weak relationship between the average net price and repayment rate. 


pctfloan_rpy3yr_scatter:
is a scatterplot of 3 yr repayment rate vs percent of students who received federal student loans. By looking the graph, it shows that there is weak relationship between percent of students who recieved federal student loans and 3yr repayment rate.
