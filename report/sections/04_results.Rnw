...
coef.df <- data.frame(rpy_3yr_regsum$Coefficients, rpy3yr_rr_coef, rpy3yr_lr_coef, rpy_pcr_coef, rpy_pls_coef)
colnames(coef.df) <- c('OLS', 'Ridge', 'Lasso', 'PCR', 'PLSR')
coef_latex <- xtable(coef.df, caption = 'Coefficients for each regression function', digits=c(0,5,5,5,5,5))
print(coef_latex, comment = FALSE, table.placement='H', type='latex', caption.placement = 'top')
@
